...
const main = async () => {
    ...
    // ============ USERS ============
    const user = await User.createWithHashedPassword({ username: 'lb', firstName: 'laurent', lastName: 'bubba', email: 'lb@lb.lb', password: 'lb' });

    const lb = userService.createUser(user)
    ...
    await prisma.recipe.create({
        data: {
            ...
            cookingDescription: 'A creamy Roman classic.',
            ...
            steps: {
                create: [
                    ...
                    },
                    {
                        order: 2,
                        title: 'Prepare Sauce',
                        description: 'Whisk eggs and cheese. Fry guanciale until crisp.',
                        time: 8,
                        ingredients: {
                            create: [
                                { name: 'Eggs', quantity: 3, unit: 'pieces' },
                                { name: 'Pecorino Romano', quantity: 50, unit: 'grams' },
                                { name: 'Guanciale', quantity: 150, unit: 'grams' }
                            ]
                        }
                    }
                ]
    ...
    await prisma.recipe.create({
        data: {
            name: 'Street Tacos',
            cookingDescription: 'Quick and spicy beef tacos.',
            type: { connect: { id: mainCourse.id } },
            steps: {
                create: [
                    {
                        order: 1,
                        title: 'Brown Beef',
                        description: 'Fry the ground beef with spices until fully cooked.',
                        time: 12,
                        ingredients: {
                            create: [
                    ...
                    },
                    {
                        order: 2,
                        title: 'Assemble',
                        description: 'Warm the shells and fill with beef and toppings.',
                        time: 5,
                        ingredients: {
                            create: [
                                { name: 'Corn Tortillas', quantity: 8, unit: 'pieces' },
                                { name: 'Cilantro', quantity: 20, unit: 'grams' }
                            ]
                        }
                    }
                ]
    ...
    await prisma.recipe.create({
        data: {
            ...
            cookingDescription: 'Check steps for gooey center timing.',
                ...
                create: [
                    {
                        order: 1,
                        title: 'Melt',
                        description: 'Melt butter and chocolate together in a bain-marie.',
                        time: 5,
                        ingredients: {
                            create: [
                                { name: 'Dark Chocolate', quantity: 150, unit: 'grams' },
                    ...
                    },
                    {
                        order: 2,
                        title: 'Bake',
                        description: 'Fold in flour and sugar, then bake at 200°C.',
                        time: 12,
                        ingredients: {
                            create: [
